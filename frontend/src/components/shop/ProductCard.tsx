  ...
  return (
    <article className="group relative">
      <Link to={`/shop/${product.slug}`}>
        <div className="relative aspect-3/4 overflow-hidden bg-cream-200 mb-3 lg:mb-5">
          ...
          <button
            ...
            disabled={isLoading}
            className={`absolute top-3 right-3 lg:top-4 lg:right-4 p-1.5 lg:p-2 transition-all duration-300 ${isSaved ? 'bg-charcoal text-cream' : 'bg-cream/80 text-charcoal lg:opacity-0 lg:group-hover:opacity-100'} hover:bg-charcoal hover:text-cream disabled:opacity-50`}
            aria-label={isSaved ? 'Remove from saved items' : 'Save item'}
          >
            <Heart size={16} strokeWidth={1.5} fill={isSaved ? 'currentColor' : 'none'} />
          </button>
        </div>
        <div className="space-y-1 pt-3 lg:pt-4">
          <p className="font-serif text-base lg:text-lg text-cream leading-tight group-hover:text-cream/70 transition-colors duration-300">{product.name}</p>
          {sizes.length > 0 && <p className="text-[10px] lg:text-xs font-sans text-cream/40 tracking-wider">{sizes.join('  ·  ')}</p>}
          <div className="flex items-center gap-2 lg:gap-3 pt-0.5 lg:pt-1">
            <span className="text-xs lg:text-sm font-sans text-cream/70">{formatPrice(product.price)}</span>
            {product.compareAtPrice && <span className="text-xs lg:text-sm font-sans text-cream/35 line-through">{formatPrice(product.compareAtPrice)}</span>}
          </div>
        </div>
      </Link>
    </article>
